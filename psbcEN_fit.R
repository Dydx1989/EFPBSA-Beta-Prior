...
source("UpdateRP.R")
source("UpdateTau.R")
source("UpdateSigma.EN.R")
source("UpdateLambda1.EN.R")
source("UpdateLambda2.EN.R")
...
psbcEN.fit <-function(survObj, priorPara, initial, rw = FALSE, mcmcPara, num.reps, thin, chain = 1, save = 1000){
    ...
    pred_risk <- rowSums(survObj$x %*% t(beta.p))
    true_risk <- survObj$t
...
#fitEN <- psbcEN.fit(survObj, priorPara, initial, rw=FALSE, mcmcPara, 
